...
\date{Due by email at the date on the website, \the\year{} \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\inthenotessubproblem{Prove Jeffrey's invariance principle i.e. prove that the Jeffrey's prior makes your prior probability immune to transformations.}\spc{8}

\hardsubproblem{Explain the advantage of Jeffrey's prior in your own words.}\spc{3}

\inthenotessubproblem{What is the Jeffrey's prior for $\theta$ under the binomial likelihood? Your answer must be a distribution. You do note need to derive it as we derived it in class.}\spc{1}

\hardsubproblem{Derive Jeffrey's prior for the reparameterization $\theta = g^{-1}(r) = \frac{e^r}{1 + e^r}$ (i.e. the log-odds reparameterization) under the binomial likelihood.}\spc{13}

\hardsubproblem{[MA] Show that the prior in the previous question is a \href{https://en.wikipedia.org/wiki/Generalized_logistic_distribution}{Type IV logistic distribution} and is conjugate for the binomial likelihood with the log-odds reparameterization.}\spc{6}


\easysubproblem{Transform this prior back to a function of $\theta$ using change of variables from 340. Is it the same as the prior in (c)? Does this confirm the invariance relationship in Jeffrey's prior construction procedure?}\spc{4}
