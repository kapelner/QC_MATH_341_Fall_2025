...
\iftoggle{solutions}{\inred{
Note $f(\theta) \propto e^{a\theta -be^{\theta}}$. We can now piggyback from part (l) and start here:
...
For the remainder of this problem, let \texttt{qlgamma}($q$, $a$, $b$) be the function that returns the $q$th quantile of the rv \text{LogGamma}($a$, $b$) and \texttt{plgamma}($x$, $a$, $b$) be the CDF $F(x)$ of the rv \text{LogGamma}($a$, $b$).
...
\iftoggle{solutions}{\inred{
For this we need the delta method. In a previous question we found that $\thetahathatmle = \natlog{\xbar}$. We have $\theta = g(\lambda) = \natlog{\lambda}$ and thus $g'(\lambda) = \lambda^{-1}$. You either have proved or can prove quickly that $\doublehat{\lambda}^{MLE} = \xbar$. Hence $\sd{\hat{\lambda}^{MLE}} = \frac{\sigsq}{n} = \frac{\lambda}{n}$ and thus $\doublehat{\mathbb{S}\text{D}}\bracks{\hat{\lambda}^{MLE}} = \frac{\xbar}{n}$. We now have everything we need for the formula:

\beqn
CI_{\theta, 95\%} \approx \bracks{\thetahathatmle \pm 1.96\, g'\parens{\doublehat{\lambda}^{MLE}} \doublehat{\mathbb{S}\text{D}}\bracks{\hat{\lambda}^{MLE}}} = \bracks{\natlog{\xbar} \pm 1.96 \parens{\oneover{\xbar}} \parens{\frac{\xbar}{n}}} = \bracks{\natlog{\xbar} \pm  \frac{1.96}{n}}
\eeqn
}}{~\spc{9}}
