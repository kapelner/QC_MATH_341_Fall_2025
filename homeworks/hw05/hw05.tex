...
\date{Due by email at the date on the homepage \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\intermediatesubproblem{Will this test be an \emph{exact test} or instead an \emph{approximate test}? Explain.}\spc{1}
...
\easysubproblem{State the univariate delta method. List assumptions.}\spc{2}

\easysubproblem{Prove the univariate delta method from the assumptions. Justify each step.}\spc{10}

\intermediatesubproblem{Assume the $\iid$ Bernoulli DGP with mean $\theta$. Let $\phi := \exp{\theta}$. Derive an asymptotically normal estimator for $\phi$.}\spc{6.5}

\easysubproblem{Assume the $\iid$ Bernoulli DGP with mean $\theta$. Sometimes researchers are interested in the following parameterization: the log odds against the event ocurring, i.e. $\phi := \natlog{\frac{1-\theta}{\theta}}$, a metric that can be any number in $\reals$. Derive an asymptotically normal estimator for $\phi$.}\spc{6.5}

\intermediatesubproblem{Given the previous answer, write a formula for $\doublehat{CI}_{\phi, 1-\alpha}$ where $\phi$ is the log odds against the event ocurring.}\spc{5}
...
\problem{We will have an example of the $F$ test for homogeneity of variance. We will use the same height data from class. The female population is considered population \#1 and the male population is  considered population \#2. We assume the DGP for female height measurements is $\iid \normnot{\theta_1}{\sigsq_1}$ independent of the DGP for male height measurements assumed to be $\iid \normnot{\theta_2}{\sigsq_2}$. The sample sizes, point estimates for the mean and point estimates for the variance computed from an in-class student survey are:
...
\easysubproblem{What is the null hypothesis and the alternate hypothesis for this test?}\spc{2}

\easysubproblem{What is the test statistic that gauges departure from the null?}\spc{2}

\intermediatesubproblem{What is the null distribution where this test statistic is realized?}\spc{2}

\intermediatesubproblem{Run the test and write a concluding sentence. Note that the quantile of the $F$ distribution with 5 and 9 degrees of freedom at 95\% is $f_{5,9,95\%} = 3.482$.}\spc{6}
...
\hardsubproblem{[MA] Here's frequency data on men and women's hair color:
...
We wish to run a $\chi^2$ test of homogeneity. We don't cover this in class every year. Write the hypotheses below and run the test at $\alpha = 5\%$ and provide a concluding sentence.  Note that $F_{\chisq{3}}(7.81) = 95\%$.}\spc{15} 
...
\begin{itemize}
\item[MOD 1:] $ \normnot{0.050}{0.303}$ and AIC = 20.427
\item[MOD 2:] $ \text{Cauchy}(-0.182, 0.391)$ and AIC = 26.899
\item[MOD 3:] $ \text{Logistic}(0.028, 0.345)$ and AIC = 21.689
\item[MOD 4:] $ \text{Laplace}(-0.176,0.496)$ and AIC = 23.843
\end{itemize}
...
\vspace{-0.5cm}
\intermediatesubproblem{Compute $\ell\parens{\thetahathatmle_1, \thetahathatmle_2; x_1, \ldots, x_{10}}$ for MOD 1 by using the AIC value. Remember $\theta_2$ in the $ \normnot{\theta_1}{\theta_2}$ notation is the variance (i.e., not the standard deviation)!}\spc{1}


\intermediatesubproblem{Compute $\ell\parens{\thetahathatmle_1, \thetahathatmle_2; x_1, \ldots, x_{10}}$ for MOD 3 without using the AIC value This requires some computation.}\spc{10}
